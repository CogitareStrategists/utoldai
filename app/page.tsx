...
>
  ...
  <nav className="flex items-center gap-5 text-[10px] uppercase tracking-[0.28em] text-white/50">
    ...
    <Link href="/#contact">Contact</Link>
        ...
        <div className="relative mx-auto mt-4 grid min-h-[calc(100vh-6.5rem)] max-w-7xl items-stretch gap-10 lg:grid-cols-12 lg:gap-0">
          <motion.div initial={{ opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.85, delay: 0.1, ease: [0.22, 1, 0.36, 1] }} className="relative z-10 flex items-center py-10 lg:col-span-5 lg:py-20">
            <div className="max-w-3xl">
              ...
              <h1 className="max-w-4xl text-5xl font-semibold uppercase leading-[0.84] tracking-[-0.075em] text-white sm:text-6xl lg:text-[5.15rem] xl:text-[5.85rem] [font-family:Inter_Tight,Inter,ui-sans-serif,system-ui,sans-serif]">Plan your AI future with judgment.</h1>
              ...
              <div className="mt-12 flex flex-col gap-4 sm:flex-row sm:flex-wrap">
                <a href="#applications" className="inline-flex items-center justify-center rounded-full bg-white px-7 py-4 text-sm font-semibold uppercase tracking-[0.24em] text-slate-950 transition duration-300 hover:-translate-y-0.5 hover:shadow-[0_12px_40px_rgba(255,255,255,0.18)]">View Applications</a>
                <a href="tel:+919182626500" className="inline-flex items-center justify-center rounded-full border border-white/15 bg-white/5 px-7 py-4 text-sm font-semibold uppercase tracking-[0.2em] text-white transition duration-300 hover:-translate-y-0.5 hover:bg-white/10">Call +91 9182626500</a>
              </div>
            </div>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.985 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.95, delay: 0.15, ease: [0.22, 1, 0.36, 1] }} className="relative mt-2 min-h-[440px] overflow-hidden rounded-[2rem] lg:col-span-7 lg:mt-0 lg:min-h-full xl:rounded-[2.75rem]">
            <img src="https://images.unsplash.com/photo-1504384308090-c894fdcc538d?auto=format&fit=crop&w=2000&q=80" alt="Premium AI consulting visual" className="absolute inset-0 h-full w-full scale-[1.03] object-cover" />
      ...
      <section id="applications" className="px-5 pb-8 pt-10 sm:px-8 sm:pt-14 lg:px-12 lg:pb-8 lg:pt-20">
        <motion.div {...fadeUp} className="mx-auto max-w-7xl">
          ...
          <div className="max-w-4xl">
            <div className="text-[11px] uppercase tracking-[0.36em] text-cyan-100/60">Applications</div>
            <h2 className="mt-6 text-4xl font-semibold leading-[0.92] tracking-[-0.06em] text-white sm:text-5xl lg:text-[4.25rem] [font-family:Inter_Tight,Inter,ui-sans-serif,system-ui,sans-serif]">Where are you applying AI?</h2>
            <p className="mt-6 max-w-2xl text-lg leading-relaxed text-white/66 sm:text-xl lg:text-[1.3rem]">AI Consulting is not one thing. The right decisions depend on where AI enters your world.</p>
          </div>
        </motion.div>
        <div className="mx-auto mt-12 grid max-w-7xl gap-8 lg:grid-cols-3">
          {applications.map((item, index) => (
            <motion.a key={item.title} href={item.href} {...fadeUp} transition={{ duration: 0.72, delay: index * 0.05, ease: [0.22, 1, 0.36, 1] }} className="group relative overflow-hidden rounded-[2rem] border border-white/10 bg-white/[0.045] shadow-[0_24px_80px_rgba(0,0,0,0.35)]">
              <div className="relative h-[24rem] sm:h-[28rem] lg:h-[32rem]">
                <img src={item.image} alt={item.title} className="absolute inset-0 h-full w-full scale-[1.02] object-cover transition duration-700 group-hover:scale-[1.06]" />
                ...
                <div className="absolute bottom-0 left-0 right-0 p-6 sm:p-8">
                  <div className="text-[11px] uppercase tracking-[0.32em] text-cyan-100/60">Application</div>
                  <h3 className="mt-4 text-3xl font-semibold leading-[0.95] tracking-[-0.05em] text-white sm:text-4xl [font-family:Inter_Tight,Inter,ui-sans-serif,system-ui,sans-serif]">{item.title}</h3>
                  <p className="mt-3 text-base leading-relaxed text-white/70 sm:text-lg">{item.description}</p>
                  <div className="mt-6 text-[11px] uppercase tracking-[0.28em] text-white/72">Enter</div>
                </div>
              </div>
            </motion.a>
          ))}
                ...
                <div className={`flex items-center p-7 sm:p-9 lg:col-span-6 lg:p-14 ${index % 2 === 1 ? 'lg:order-2' : ''}`}>
                  <div>
                    ...
                    <div className="mt-8 space-y-5 lg:space-y-6">
                      <h2 className="text-3xl font-semibold leading-[0.95] tracking-[-0.055em] text-white sm:text-4xl lg:text-[3.35rem] [font-family:Inter_Tight,Inter,ui-sans-serif,system-ui,sans-serif]">{section.title}</h2>
                      <p className="max-w-2xl text-lg leading-relaxed text-white/66 sm:text-xl lg:text-[1.35rem]">{section.line}</p>
                      <p className="pt-4 text-2xl font-medium leading-tight tracking-[-0.03em] text-cyan-50 sm:text-3xl lg:text-[2.5rem]">{section.question}</p>
                    </div>
                  </div>
                </div>
      ...
      <section id="contact" className="px-5 pb-24 pt-10 sm:px-8 lg:px-12 lg:pb-28 lg:pt-16">
          ...
          <div className="grid gap-8 px-6 py-10 sm:px-8 sm:py-14 lg:grid-cols-12 lg:px-14 lg:py-16">
            <div className="lg:col-span-8">
              ...
              <div className="mt-6 space-y-4 sm:space-y-5">
                <p className="text-xl text-white/72 sm:text-2xl lg:text-3xl">Your seriousness is our strength</p>
                <p className="text-xl text-white/72 sm:text-2xl lg:text-3xl">If you have the intent, we can fuel the creation</p>
                <h2 className="max-w-4xl text-3xl font-semibold leading-[0.96] tracking-[-0.055em] text-white sm:text-5xl lg:text-[4rem] [font-family:Inter_Tight,Inter,ui-sans-serif,system-ui,sans-serif]">To work together on your AI future</h2>
              </div>
            </div>
